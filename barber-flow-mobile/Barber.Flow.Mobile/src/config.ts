    ...
    export const APP_ENV = (process.env.APP_ENV as string | undefined) ?? expoExtra?.APP_ENV ?? "development";

    export const BASE_URL = (process.env.BARBERFLOW_API_URL as string | undefined) ??
        expoExtra?.BASE_URL ?? "https://barberflowapp-develop.up.railway.app";

    // Admin username (single admin). Can be configured via env or Expo extra.
    export const ADMIN_USERNAME = (process.env.BARBERFLOW_ADMIN_USERNAME as string | undefined) ?? expoExtra?.ADMIN_USERNAME ?? "admin";