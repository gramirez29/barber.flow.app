...
const buildDelayedClientId = (clientKey: string) =>
	`delayed-client-summary:${clientKey}`;
...
const buildNextDaySummary = (
	...
	const tomorrowAppointments = appointments.filter(
		(appointment) =>
		isScheduledAppointment(appointment) && appointment.date === date,
	);
...
const buildClientHistories = (
	...
	appointments.filter(isScheduledAppointment).forEach((appointment) => {
		...
		current.hasFutureAppointment =
		current.hasFutureAppointment || appointment.date > todayKey;
...
const buildDelayedClientSummaries = (
	...
	histories.forEach((history) => {
		...
		const daysSinceLastAppointment = differenceInCalendarDays(
			startOfDay(today),
			lastAppointment,
		);
