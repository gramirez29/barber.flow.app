...
public class MongoDbReportRepositoryTests
{
    ...
    [Fact]
    public async Task GetDailyReportAsync_RequestingBarberHasShop_FiltersAppointmentsByShopId()
    {
        ...
        _appointmentRepository
            .Setup(r => r.FindAsync("2026-08-19", "2026-08-19", "completed", null, null, null, "SHOP-0001", It.IsAny<CancellationToken>()))
            .ReturnsAsync([]);

        await CreateSut().GetDailyReportAsync(reportDate, "barber1");

        _appointmentRepository.Verify(
            r => r.FindAsync("2026-08-19", "2026-08-19", "completed", null, null, null, "SHOP-0001", It.IsAny<CancellationToken>()),
            Times.Once);
    }

    [Fact]
    public async Task GetDailyReportAsync_NoLinkedBarber_DoesNotFilterByShop()
    {
        var reportDate = new DateOnly(2026, 8, 19);
        _barberRepository.Setup(r => r.GetByUserNameAsync("admin", It.IsAny<CancellationToken>())).ReturnsAsync((BarberEntity?)null);
        _appointmentRepository
            .Setup(r => r.FindAsync("2026-08-19", "2026-08-19", "completed", null, null, null, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync([]);

        await CreateSut().GetDailyReportAsync(reportDate, "admin");

        _appointmentRepository.Verify(
            r => r.FindAsync("2026-08-19", "2026-08-19", "completed", null, null, null, null, It.IsAny<CancellationToken>()),
            Times.Once);
    }
}
